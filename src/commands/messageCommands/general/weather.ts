...
/* Replace URLs later with your own assets */
...
const weather: MessageCommand = {
  ...
  async executeMessage(message: Message, args: string[]) {
    ...
    try {
      ...
      if (data.cod !== 200 || !data.main || !data.weather?.length) {
...
Sunset: ${formatTime(data.sys.sunset)}`,
      ...
      console.error('[Weather Command]', error)

      const errorEmbed = new EmbedBuilder()
        .setColor(COLORS.red)
        .setDescription(
          'Unable to fetch weather information. Please check the location or try again later.',
        )
